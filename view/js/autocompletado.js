...
function cargarvalores() {
    $.ajax({ 
  type: "POST", 
  url: "../controller/producto.cargar.controller.php", 
...
function autocomplete(inp, arr) {
  ...
  inp.addEventListener("input", function (e) {
    ...
    for (i = 0; i < arr.length; i++) {
      ...
      if (
        (arr[i].descripcion.toUpperCase()).search(val.toUpperCase()) != -1
      ) {
        /*create a DIV element for each matching element:*/
        b = document.createElement("DIV");
        /*make the matching letters bold:*/
       b.innerHTML = arr[i].descripcion;
        ...
        /*b.innerHTML +=
          ...
          */
            ...
            des_producto[i] = arr[i].descripcion;
            prod_id[i] = arr[i].id_producto;
        /*execute a function when someone clicks on the item value (DIV element):*/
        b.addEventListener("click", function (e) {
          var producto;
          var p_prod_id;
          for (i = 0; i < arr.length; i++) {
            if(des_producto[i] === e.currentTarget.innerText){
              producto = des_producto[i];
              p_prod_id = prod_id[i]
              location.href ='productos.agregar.carrito.view.php?id='+p_prod_id+'';
            }
          }
